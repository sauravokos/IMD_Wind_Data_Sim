...
class PlottingGUI(tk.Frame):
    ...
    def show_barbs1(self):
        ...
        for i in range(0, self.HaM.shape[0]):
            if self.HaM[i] == 0:
                self.brbn.choose_barb(self.WiS[i], "~\AppData\Local\Programs\Python\Python36-32\projects\Red_Barbs\\")
                vec_im = tk.PhotoImage(file=self.brbn.BrbNm).subsample(2)
    ...
    def show_barbs2(self):
        ...
        for i in range(0, self.HaM.shape[0]):
            if self.HaM[i] == 1:
                self.brbn.choose_barb(self.WiS[i], "~\AppData\Local\Programs\Python\Python36-32\projects\Green_Barbs\\")
                vec_im = tk.PhotoImage(file=self.brbn.BrbNm).subsample(2)
                self.coord.find_px(self.Lat[i], self.Lon[i])
                x = self.coord.LonPx
                y = self.coord.LatPx
    ...
    def show_barbs3(self):
        ...
        for i in range(0, self.HaM.shape[0]):
            if self.HaM[i] == 2:
                self.brbn.choose_barb(self.WiS[i], "~\AppData\Local\Programs\Python\Python36-32\projects\Blue_Barbs\\")
                vec_im = tk.PhotoImage(file=self.brbn.BrbNm).subsample(2)
                self.coord.find_px(self.Lat[i], self.Lon[i])
                x = self.coord.LonPx
                y = self.coord.LatPx
